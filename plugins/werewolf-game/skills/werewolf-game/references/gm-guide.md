...
- **発言を受け取ったら必ず即座に更新すること**（コンパクション対策）。broadcast の前後を問わず、受け取り次第1件ごとに書き込む
- 1日の終了時には必ず完成させること
...
【1日目の朝】
朝起きると、平和な夜が待っていました。
それでは、{最初の発言者}さんから順に議論を開始してください。
```

**2日目以降で犠牲者なしの場合（護衛成功または妖狐を襲撃）:**
```
【{N}日目の朝】
朝起きると、平和な夜が待っていました。
...
以下の情報はGMのみが知る秘密情報。プレイヤーには**絶対に開示してはならない**:

| 情報 | 説明 |
|------|------|
| 配役 | 各プレイヤーの役職（人狼・占い師等） |
| 占い結果 | 占い師が占った対象と結果 |
| 霊能結果 | 霊能者への通知内容 |
| 護衛先 | 狩人が護衛した対象 |
| 襲撃先 | 人狼が選んだ襲撃対象 |
...
- 秘匿情報は必ず **SendMessage（個別メッセージ）** で送信する
- **broadcast で秘匿情報を送ることは絶対禁止**
- GMが broadcast するのは公開情報のみ（処刑結果、生存者情報、発言内容など）

### 誤開示した場合の廃村処理

万が一、秘匿情報を誤って broadcast してしまった場合は、**直ちにゲームを中断**する:

1. 即座に以下をbroadcastで廃村宣言:
   ```
   【廃村】
   GMの重大なミスにより、ゲームを続行できません。廃村とします。
   理由: [開示してしまった秘匿情報の種別]
   ```
2. 全生存者に shutdown_request を送信
3. 全員の承認後、TeamDelete を実行
4. `gm/memo.md` に廃村の記録を残す（日時・開示した情報・理由）
