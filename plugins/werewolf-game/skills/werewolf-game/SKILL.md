---
name: werewolf-game
description: "人狼ゲームをAgent Teamsで実行する。'/werewolf-game' で起動。Team Lead がGM（ゲームマスター）となり、9人のチームメイトをプレイヤーとしてスポーンし、ターン制の人狼ゲームを進行する。配役構成は 9A（標準）/9B（霊能なし）/9C（妖狐あり）から選択可能。"
...
references/characters/ 配下の12枚のハンドアウトから9枚をランダムに選択する。被りなし。

選択ロジック:
1. Glob で references/characters/*.md を列挙
2. `shuf` コマンドでランダムに9枚を選ぶ:
   ```bash
   ls references/characters/*.md | shuf -n 9
   ```
3. 選んだ9枚を Read で読み込む
...
**日次ログへの記録**: 各発言を受け取ったら（またはbroadcast確認後）、原文そのまま day-{N}.yaml に追記する。**こまめに書き込むこと**（コンパクション対策）。
